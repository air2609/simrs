...
public class RacikanController extends BaseController{
...
//	private MsItemSellingPrice itemSellingPrice;
	...
	public void init(Component win) throws InterruptedException, VONEAppException {
		...
		itemRoot = (Listbox)session.getAttribute("listbox");
		this.win = win;
		...
		isRajal = (boolean)win.getAttribute("isRajal");
//		System.out.println("rajalkah? " + isRajal);
	...
	public void save() throws VONEAppException, InterruptedException{
		...
		while(it.hasNext()){
			...
			if(isRajal){
				...
				Double nilaiPPn = ppn.doubleValue() * hargaObat;
				hargaObat = hargaObat + nilaiPPn.doubleValue();
			}
		...
		decimalbox.addEventListener("onChange", new DiscontListener(listitem,diskonList,finalPrice,decimalbox,listcell,quantity,new Short("0")));
		diskonList.addEventListener("onSelect", new DiscontListener(listitem,diskonList,finalPrice,decimalbox,listcell, quantity,new Short("0")));
